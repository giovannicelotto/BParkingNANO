...
config.General.workArea = 'DataFiltered{:s}'.format(production_tag)
  ...
  with open(args.yaml) as f:
    ...
    for sample, info in doc['samples'].items():
      ...
      for part in parts:
        ...
        config.JobType.outputFiles = ['_'.join(['DataFiltered', 'Run3' if args.lhcRun==3 else 'Run2', 'mc' if isMC else 'data', production_tag])+'.root']
