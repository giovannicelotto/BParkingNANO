...
void ParticleNetFeatureEvaluator::fillParticleFeatures(DeepBoostedJetFeatures & fts, 
  ...
  for (const auto & icand : daughterIndices) {
    ...
    fts.fill("jet_pfcand_dz", ip_sign_*(std::isnan(cand->dz(pv_ass_pos)) ? 0 : ip_sign_*cand->dz(pv_ass_pos)));
    fts.fill("jet_pfcand_dxy", ip_sign_*(std::isnan(cand->dxy(pv_ass_pos)) ? 0 : cand->dxy(pv_ass_pos)));
    ...
    if (track) {
      fts.fill("jet_pfcand_dzsig", ip_sign_*(std::isnan(fabs(cand->dz(pv_ass_pos)) / cand->dzError()) ? 0 : fabs(cand->dz(pv_ass_pos)) / cand->dzError()));
      fts.fill("jet_pfcand_dxysig", ip_sign_*(std::isnan(fabs(cand->dxy(pv_ass_pos)) / cand->dxyError()) ? 0 : fabs(cand->dxy(pv_ass_pos)) / cand->dxyError()));
      ...
      fts.fill("jet_pfcand_trackjet_d3d", ip_sign_*(std::isnan(ip_sign_*meas_ip3d.value()) ? 0 : meas_ip3d.value()));
      fts.fill("jet_pfcand_trackjet_d3dsig", ip_sign_*(std::isnan(ip_sign_*fabs(meas_ip3d.significance())) ? 0 : fabs(meas_ip3d.significance())));
      fts.fill("jet_pfcand_trackjet_dist", ip_sign_*(std::isnan(-meas_jetdist.value()) ? 0 : -meas_jetdist.value()));
      fts.fill("jet_pfcand_trackjet_decayL", ip_sign_*(std::isnan(meas_decayl.value()) ? 0 : meas_decayl.value()));      
    } 
    else {
...
void ParticleNetFeatureEvaluator::fillSVFeatures(DeepBoostedJetFeatures &fts, 
  ...
  for (const auto *sv : jetSVs) {
    ...
    fts.fill("jet_sv_dxy", ip_sign_*(std::isnan(valxy.value()) ? 0 : valxy.value()));
    fts.fill("jet_sv_dxysig", ip_sign_*(std::isnan(fabs(valxy.significance())) ? 0 : fabs(valxy.significance())));
    
    VertexDistance3D d3d;
    auto val3d = d3d.signedDistance(svtx, *pv_, jet_global_vec);
    fts.fill("jet_sv_d3d", ip_sign_*(std::isnan(val3d.value()) ? 0 : val3d.value()));
    fts.fill("jet_sv_d3dsig", ip_sign_*(std::isnan(fabs(val3d.significance())) ? 0 : fabs(val3d.significance())));
...
void ParticleNetFeatureEvaluator::fillLostTrackFeatures(DeepBoostedJetFeatures &fts, 
  ...
  for(auto const & ltrack : jet_lost_tracks){
    ...
    fts.fill("jet_losttrack_dz",ip_sign_*(std::isnan(ltrack.dz(pv_ass_pos)) ? 0 : ltrack.dz(pv_ass_pos)));
    fts.fill("jet_losttrack_dxy",ip_sign_*(std::isnan(ltrack.dxy(pv_ass_pos)) ? 0 : ltrack.dxy(pv_ass_pos)));
    ...
    if(track){
      ...
      fts.fill("jet_losttrack_dxysig",ip_sign_*(std::isnan(fabs(ltrack.dxy(pv_ass_pos))/ltrack.dxyError()) ? 0 : fabs(ltrack.dxy(pv_ass_pos))/ltrack.dxyError()));
      fts.fill("jet_losttrack_dzsig",ip_sign_*(std::isnan(fabs(ltrack.dz(pv_ass_pos))/ltrack.dzError()) ? 0 : fabs(ltrack.dz(pv_ass_pos))/ltrack.dzError()));
      ...
      fts.fill("jet_losttrack_trackjet_d3d", ip_sign_*(std::isnan(meas_ip3d.value()) ? 0 : meas_ip3d.value()));
      fts.fill("jet_losttrack_trackjet_d3dsig", ip_sign_*(std::isnan(fabs(meas_ip3d.significance())) ? 0 : fabs(meas_ip3d.significance())));
      fts.fill("jet_losttrack_trackjet_dist", ip_sign_*(std::isnan(-meas_jetdist.value()) ? 0 : -meas_jetdist.value()));
      fts.fill("jet_losttrack_trackjet_decayL", ip_sign_*(std::isnan(meas_decayl.value()) ? 0 : meas_decayl.value()));      
    }
    else{
